...
class ReversingJournalEntry(AccountsController):
	def __init__(self, *args, **kwargs):
		super(ReversingJournalEntry, self).__init__(*args, **kwargs)
	...
	def reverse_gl_entries(self, cancel=0, adv_adj=0):
		from mobile_fun_customisation.mobile_fun_customisation.reversing_general_ledger import reverse_gl_entries
			...
			if d.debit or d.credit:
				gl_map.append(
					self.get_gl_dict({
						...
						"credit": flt(d.debit, d.precision("debit")),
						"account_currency": d.account_currency,
						"debit_in_account_currency": flt(d.credit_in_account_currency, d.precision("credit_in_account_currency")),
						"credit_in_account_currency": flt(d.debit_in_account_currency, d.precision("debit_in_account_currency")),
						"against_voucher_type": d.reference_type,
						"against_voucher": d.reference_name,
						"remarks": self.remark,
						"cost_center": d.cost_center,
		...
		if gl_map:
			reverse_gl_entries(gl_map, cancel=cancel, adv_adj=adv_adj)
...
def get_against_jv(doctype, txt, searchfield, start, page_len, filters):
	return frappe.db.sql("""select jv.name, jv.posting_date, jv.reversing_date, jv.user_remark
		from `tabReversing Journal Entry` jv, `tabJournal Entry Account` jv_detail
